...
class SyncDialog(wx.Dialog):
    ...
    def build_files(self):
        ...
        label_upload = "Enviar pasta de rota" if self.file_type == "routes" else "Enviar pasta de veículo"
        self.btn_upload = wx.Button(self.panel_files, label=label_upload)
        self.btn_upload.Hide()
    ...
    def on_download(self, event):
        ...
        result = client.download(file_name, save_path, file_type=self.file_type)  # <-- corrigido
        if result.get("success"):
    ...
    def on_upload(self, event):
        default_dir = ROUTES_DIR if self.file_type == "routes" else VEHICLES_DIR
        label = "Selecione a pasta da rota" if self.file_type == "routes" else "Selecione a pasta do veículo"

        with wx.DirDialog(self, label, defaultPath=str(default_dir), style=wx.DD_DIR_MUST_EXIST) as dirDialog:
            if dirDialog.ShowModal() == wx.ID_CANCEL:
                return
            path = dirDialog.GetPath()
            result = client.upload(path, constantes.SESSION["user_id"], file_type=self.file_type)  # <-- corrigido
            if result.get("success"):
    ...
    def on_delete(self, event):
        ...
        confirm = wx.MessageBox(
            f"Tem certeza que deseja deletar '{file_name}' do servidor?",
            "Confirmar deleção",
            wx.YES_NO | wx.NO_DEFAULT | wx.ICON_WARNING
        )
        if confirm != wx.YES:
            return
        result = client.delete(file_name, constantes.SESSION["user_id"], file_type=self.file_type)  # <-- corrigido
        if result.get("success"):
