...
def upload(dir_path, user_id, file_type="routes"):
    try:
        ...
        with open(zip_path, "rb") as f:
            r = requests.post(
                f"{constantes.BASE_URL}/upload/{file_type}",
                files={"file": f},
...
def download(file_name, save_path, file_type="routes"):
    try:
        url = f"{constantes.BASE_URL}/files/{file_type[:-1]}"  # "routes" -> "route", "vehicles" -> "vehicle"
        r = requests.get(url)
        ...
        file_url = f"{constantes.BASE_URL}/data/{file_type}/{file_name}"
